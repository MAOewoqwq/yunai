  ...
  return (
    <main className="min-h-[100dvh] p-4 sm:p-6">
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <p className="text-white/70 text-sm">支持上传到 `public/uploads/bg|avatars|sprites|items`。立绘可按角色 `charId` 分类至 `sprites/charId/`。</p>
        ...
        <PixelPanel className="p-4">
          <div className="flex items-center justify-between mb-3">
            <div className="text-lg">物品图标（items）</div>
            <div className="flex items-center gap-2">
              <input ref={itemInputRef} type="file" multiple accept="image/*" />
              <button className="px-3 py-1 border hover:bg-white/10" onClick={() => upload('items')}>上传</button>
            </div>
          </div>
          <div className="text-xs text-white/60 mb-2">建议使用透明底 PNG，文件名包含物品 ID（如 gift_flowers.png），用于与背包道具匹配。</div>
          <div className="grid grid-cols-4 sm:grid-cols-6 md:grid-cols-8 gap-3">
            {itemList.map((a) => (
              <div key={a.url} className="border border-white/15 bg-white/5">
                <img src={a.url} alt={a.name || 'item'} className="w-full h-16 object-contain bg-black/30" />
                <div className="text-[10px] p-1 truncate">{a.name}</div>
              </div>
            ))}
            {itemList.length === 0 && (
